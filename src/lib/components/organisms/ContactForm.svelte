...
>
  ...
  <div class="flex flex-wrap gap-4">
    ...
    />
    <button
      class="block flex-1 basis-64 overflow-x-hidden text-nowrap rounded-xl border-2 border-mantle bg-base p-4 text-left text-lg text-surface1 placeholder-surface1 ring-surface0 duration-200 hover:ring-2 active:bg-mantle"
  ...
  <textarea
    ...
    required
    rows="10"
    class="resize-none rounded-xl border-2 border-mantle bg-crust p-4 text-lg text-text placeholder-surface1"
    name="message"
    placeholder="Message"
  />
  <div class="grid h-fit grid-cols-2 gap-6 sm:grid-cols-3">
    <a
      ...
      class="m-auto block h-fit w-full rounded-xl border-2 border-lavender p-4 text-center text-lavender sm:w-fit"
      >Open in mail</a
    >
    <button
      type="submit"
      class="m-auto h-fit w-full rounded-xl bg-lavender p-4 text-lg text-crust"
      >Send</button
    >
  </div>
</form>
